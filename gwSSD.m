function [SSDcomponents]=gwSSD(v,Fs,th,maxNumberofComponents)
...
while (remen > th) && (k1 < maxNumberofComponents)
    ...
    if ((k1 == 1) && (ff(in3)/Fs < 1e-3)) % trend detection at first iteration
        ...
        for cont = 1:2
            ...
            % findpeaks to replace gaussfitSSD
            [~,locs,w,~] = findpeaks(nmmt);
            
            % only focusing on the biggest peak in the residual
            peakloc = find(locs==in3, 1);
            
            % taking half the width to get deltafp
            deltafp = w(peakloc)/2;
            
            %estimation of bandwidth
            [~,iiii1] = min(abs(ff-(ff(in3)-deltafp)));
            [~,iiii2] = min(abs(ff-(ff(in3)+deltafp)));
